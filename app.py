...
class MultiGroupBroadcastSMS:
    ...
    def broadcast_with_media(self, from_phone, message_text, media_urls, message_type='broadcast'):
        """FIXED: Send message WITH media to EVERYONE across ALL 3 groups"""
        ...
        # FIXED: Send to ALL members with proper MMS handling
        sent_count = 0
        failed_count = 0
        mms_sent = 0
        sms_sent = 0
        ...
        for recipient in all_recipients:
            ...
            try:
                if media_urls:
                    # FIXED: Extract just the URLs (not the dict objects)
                    media_url_list = [media['url'] for media in media_urls if media.get('url')]
                    ...
                    if media_url_list:
                        ...
                        message_obj = self.client.messages.create(
                            ...
                            to=recipient['phone'],
                            media_url=media_url_list  # This was the bug - needs to be list of URLs
                        )
                        mms_sent += 1
                        print(f"📱 MMS sent to {recipient['phone']}: {message_obj.sid}")
                    else:
                        # No valid media URLs, send as SMS
                        message_obj = self.client.messages.create(
                            body=formatted_message,
                            from_=TWILIO_PHONE_NUMBER,
                            to=recipient['phone']
...
def handle_sms():
    """Handle incoming SMS/MMS from Twilio - FIXED MEDIA SUPPORT"""
    try:
        from_number = request.form.get('From', '').strip()
        message_body = request.form.get('Body', '').strip()
        
        # FIXED: Handle media properly
        media_urls = []
        ...
        print(f"❌ Error: {e}")
        return "OK", 200  # Return OK to prevent Twilio retries
...
if __name__ == '__main__':
    ...
    print("📸 Photos/audio go to ALL groups!")
